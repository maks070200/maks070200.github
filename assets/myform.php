...
if($data['counter']=='true' || $data['counter']=='1') {
	$order_id = $order_id + 1;
	$file = fopen('orders_count.txt', 'w');
	fwrite($file, $order_id);
	fclose($file);
}
if($f = fopen('formdata.csv', 'a')) {
	...
	fwrite($f, $order_id.';'.date('d.m.y H:i', time()+$config['timezone']*60).';'.$data_name.';'.$data_email.';'.$data_phone.PHP_EOL); 
	fclose($f);
}
